...
import {
  ...
  Clock, CheckCircle, BarChart2, FileCheck, Trophy, ArrowRight, Zap
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => loadData()} className={`p-2.5 rounded-xl transition-all shadow-sm ${darkMode ? 'bg-slate-800 border border-slate-700 text-slate-400 hover:bg-slate-700' : 'bg-white border border-slate-200 text-slate-500 hover:bg-slate-50'
              }`} title="Refresh Dashboard">
              <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
            </button>
            <button onClick={() => onPageChange?.('Reports')} className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center gap-2 shadow-sm ${darkMode ? 'bg-slate-800 border border-slate-700 text-slate-200 hover:bg-slate-700' : 'bg-white border border-green-200 text-green-700 hover:bg-green-50 hover:border-green-300'
              }`}>
              <FileText className={`w-4 h-4 ${darkMode ? 'text-slate-400' : 'text-green-500'}`} />
              Generate Report
            </button>
          ...
          <Card className="lg:col-span-2 flex flex-col" darkMode={darkMode}>
            <div className="mb-6 flex justify-between items-center">
              ...
              <select
                ...
                className={`border text-sm rounded-lg focus:ring-green-500 block p-2 outline-none cursor-pointer ${darkMode ? 'bg-slate-700 border-slate-600 text-slate-200' : 'bg-slate-50 border-slate-200 text-slate-700'
                  }`}
              >
          ...
          <Card darkMode={darkMode} className="flex flex-col">
            ...
            <div className="flex-1 min-h-[300px]">
              {loading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={deadlines} layout="vertical" margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                    <defs>
                    ...
                    <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fill: darkMode ? '#94a3b8' : '#475569', fontSize: 13, fontWeight: 500 }} width={80} />
                    ...
                    <Bar dataKey="progress" fill="url(#greenBar)" radius={[0, 6, 6, 0]} barSize={24} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <Card darkMode={darkMode} className="lg:col-span-1">
            ...
            <div className="space-y-3">
              <button onClick={() => onPageChange?.('Environmental')} className="w-full px-4 py-3 bg-green-500 text-white font-medium rounded-xl shadow-sm hover:bg-green-600 transition-all flex items-center justify-center gap-2">
                <Plus className="w-5 h-5" /> Log Carbon Data
              </button>
              <button onClick={() => onPageChange?.('Gamification')} className="w-full px-4 py-3 bg-orange-500 text-white font-medium rounded-xl shadow-sm hover:bg-orange-600 transition-all flex items-center justify-center gap-2">
                <Trophy className="w-5 h-5" /> Start Challenge
              </button>
              <button onClick={() => onPageChange?.('Reports')} className={`w-full px-4 py-3 font-medium rounded-xl shadow-sm transition-all flex items-center justify-center gap-2 ${darkMode ? 'bg-slate-700 text-slate-200 hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Card darkMode={darkMode}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => onPageChange?.('Environmental')} className="text-green-600 text-sm font-medium hover:text-green-700" aria-label="View all goals">View All</button>
            </div>
                ...
                <div key={goal.id}>
                  <div className="flex justify-between items-end mb-2">
                    <div>
                      ...
                      <p className={`text-xs mt-0.5 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Due: {goal.due}</p>
                    </div>
          ...
          <Card darkMode={darkMode}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => onPageChange?.('Gamification')} className="text-green-600 text-sm font-medium hover:text-green-700" aria-label="Explore challenges">Explore</button>
            </div>
            <div className="space-y-4">
              ...
              {!loading && activeChallenges.length === 0 && (
                <p className="text-slate-500 text-sm text-center py-8">No active challenges right now.</p>
              )}
            </div>
          </Card>
          ...
          <Card darkMode={darkMode} className="flex-1">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => onPageChange?.('Governance')} className="text-red-500 text-sm font-medium hover:text-red-600" aria-label="Review issues">Review</button>
            </div>
            <div className="space-y-3">

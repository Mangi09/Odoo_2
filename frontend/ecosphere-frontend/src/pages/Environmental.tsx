...
const Badge = ({ children, variant = 'default' }: { children: React.ReactNode, variant?: 'success' | 'warning' | 'error' | 'default' }) => {
  const styles = { success: 'bg-green-100 text-green-700', warning: 'bg-orange-100 text-orange-700', error: 'bg-red-100 text-red-700', default: 'bg-slate-100 text-slate-700' };
  return <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${styles[variant]}`}>{children}</span>;
};

const StatusBadge = ({ status }: { status: string }) => {
  if (status === 'Verified' || status === 'Achieved' || status === 'On Track') return <Badge variant="success">{status}</Badge>;
  if (status === 'Pending' || status === 'At Risk') return <Badge variant="warning">{status}</Badge>;
...
const TypeIcon = ({ type }: { type: string }) => {
  switch(type) {
    ...
    case 'Fleet': return <Droplets className="w-4 h-4 text-blue-500" />;
    ...
    <DashboardLayout activePage={activePage} onPageChange={onPageChange}>
      <div className="max-w-7xl mx-auto space-y-8 relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Environmental Tracking</h1>
            <p className="text-slate-500 mt-1 text-sm">Monitor carbon emissions and sustainability goals in real-time.</p>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={handleCalculate} disabled={calculating}
              className="px-4 py-2.5 bg-green-600 text-white rounded-xl text-sm font-medium hover:bg-green-700 transition-all shadow-sm shadow-green-600/20 flex items-center gap-2 disabled:opacity-70">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { icon: Cloud, label: 'Total Emissions', value: `${totalEmissions.toFixed(1)} tCO₂e`, color: 'text-slate-600', bg: 'bg-slate-50' },
            { icon: CheckCircle2, label: 'Verified', value: verifiedCount.toString(), color: 'text-green-600', bg: 'bg-green-50' },
            { icon: Target, label: 'Active Goals', value: goals.filter(g => ['ACTIVE', 'ON_TRACK'].includes(g.status)).length.toString(), color: 'text-blue-600', bg: 'bg-blue-50' },
            { icon: TrendingDown, label: 'Transactions', value: transactions.length.toString(), color: 'text-indigo-600', bg: 'bg-indigo-50' },
          ].map((s, i) => (
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:flex-none">
                ...
                <input type="text" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  className="pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none w-full sm:w-48" />
              </div>
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:border-indigo-500 outline-none">
                  ...
                  <tr key={tx.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-slate-900">{tx.source}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">{tx.category}</td>
                    <td className="px-4 py-3 text-slate-500">{tx.department}</td>
                    <td className="px-4 py-3 text-right font-semibold text-slate-900">{tx.amount.toFixed(2)}</td>
                    <td className="px-4 py-3"><StatusBadge status={tx.status} /></td>
                  </tr>
                ))}
                {!loading && filteredTransactions.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">No transactions found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <Card>
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-slate-900">Environmental Goals</h3>
            <span className="text-sm text-slate-500">{goals.filter(g => g.status === 'COMPLETED').length}/{goals.length} completed</span>
          </div>
                ...
                <div key={goal.id}>
                  <div className="flex justify-between items-end mb-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">{goal.current.toFixed(1)} / {goal.target} {goal.unit} • Due {goal.deadline}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <StatusBadge status={onTrack ? 'On Track' : goal.status === 'ACTIVE' ? 'Pending' : goal.status} />

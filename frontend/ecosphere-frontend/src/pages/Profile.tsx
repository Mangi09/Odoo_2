      ...
      <div className="max-w-7xl mx-auto space-y-6 relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => loadData()} className={`p-2.5 rounded-xl shadow-sm transition-all ${darkMode ? 'bg-slate-800 border border-slate-700 text-slate-400 hover:bg-slate-700' : 'bg-white border border-slate-200 text-slate-500 hover:bg-slate-50'
              }`} title="Refresh">
              <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
            </button>
            <button
              onClick={() => logout()}
              className={`px-4 py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 transition-all ${darkMode ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30 border border-red-500/30' : 'bg-red-50 text-red-700 hover:bg-red-100 border border-red-200'
                }`}
            >
            ...
            <Card className="h-[480px] flex flex-col" darkMode={darkMode}>
              ...
              <div className="overflow-y-auto pr-2 space-y-3 flex-1 custom-scrollbar">
                {loading ? (
                  ...
                  <div key={item.id} className={`flex items-center justify-between p-4 rounded-xl border transition-colors ${darkMode ? 'border-slate-700 hover:border-slate-600 hover:bg-slate-700/50' : 'border-gray-100 hover:border-gray-200 hover:bg-gray-50/50'
                    }`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className={`flex items-center gap-2 text-xs mt-1 ${darkMode ? 'text-slate-400' : 'text-gray-500'}`}>
                          <span className={`font-medium ${item.status === 'Completed' || item.status === 'Approved' ? 'text-green-600' :
                              'text-amber-600'
                            }`}>
                            {item.status}
                          </span>
